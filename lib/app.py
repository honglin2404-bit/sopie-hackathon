...
Updated for new database structure with 21 columns
"""
...
SUPABASE_ANON_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Inhtp2FtZHd5a2x2b2ZmemZhb3ltIiwicm9sZSI6ImFub24iLCJpYXQiOjE3MzE2NTg0NzYsImV4cCI6MjA0NzIzNDQ3Nn0.xuFJCDaYVRGk7KLlKPvNaJqStCOqwi8mUJBQVDiYYBo"
    ...
    try:
        ...
        if supabase:
            ...
            query_builder = supabase.table('sops').select(
                ...
                'template_app_mail, template_call_chat, link_sop'
            )
            ...
            params = {
                "select": "id,title,domain,product,feature,cause,solution_l1,solution_l2,keywords_primary,keywords_secondary,check_tool_guideline,check_tools_name,check_tools_url,template_app_mail,template_call_chat,link_sop",
                "limit": limit
            }
            ...
            # Add text search (simplified for requests)
            # Note: Full text search might need different approach with requests
...
def search_by_embedding(query_text, domain_filter=None, limit=5):
    ...
    This requires OpenAI API for generating query embedding
    """
    try:
        # For now, return mock results
        # In production, you would:
        # 1. Generate embedding for query_text using OpenAI
        # 2. Use pgvector to find similar embeddings
        # 3. Return matched SOPs
        ...
        logger.error(f"Error in search_by_embedding: {e}")
        return []
...
def format_sop_response(sop):
    ...
    return {
        ...
        'relevance_score': 0.95  # Mock score for now
    }
...
def search():
    ...
    try:
        ...
        search_type = data.get('type', 'keyword')  # 'keyword' or 'semantic'
